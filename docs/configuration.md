...
```toml
[display]
verbose = false                 # 補助テキストに詳細を出す（入力中に Ctrl+Shift+V で切替）
```
...
開発や設定の調整で内部の動きを確認したいときは、入力中に `Ctrl+Shift+V` を押して詳細表示に切り替えます。押した時点で表示が変わります（起動時から有効にするには `[display] verbose = true`）。何も入力していないときの `Ctrl+Shift+V` はアプリケーションにそのまま渡されるので、端末などの貼り付けは従来どおり効きます。詳細表示では次の情報が加わります。
