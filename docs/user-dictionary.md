...
読みは「ヨミ（かな）」である必要はありません。**ローマ字ルールにならない打鍵**をそのまま読みにすると、英字のショートカットとして使えます。メールアドレスや定型文の入力に便利です。

```
m	masahiro@asial.co.jp	短縮	メールアドレス(会社)
m	masahiro.tanaka@gmail.com	短縮	メールアドレス(個人)
gm	masahiro.tanaka@gmail.com	短縮	メールアドレス(個人)
```

`m` と打って Space を押すと、2つのアドレスが候補に並びます。同じ読みに複数行書けば、書いた順に候補が並びます。

**使えるのは子音だけの打鍵です。** `m`・`gm`・`ks` のように、かなに確定しない打鍵の並びが読みになります。`tel` は `te` が「て」に確定してしまうため（`てl` という読みになる）ショートカットにはなりません。

| 読み | 使えるか | 理由 |
|------|----------|------|
| `m` `gm` `ks` | ○ | 子音だけなので未確定のまま残る |
| `1` `@` `;` | ○ | ローマ字ルールを開始しないので即確定する |
| `tel` `mail` | × | 母音でかなに確定してしまう |
| `めーる` | ○ | 普通のかな読み |

2文字以上のショートカットは打った時点で候補ウィンドウに出ますが、**1文字のショートカットは Space を押してから**出ます（最初の1打鍵では候補の問い合わせが走らないため）。

なお1文字の読みは、その文字で始まる語を打ち始めるたびに候補に顔を出します（`m` を登録していると「まさひろ」の `m` でも出る）。次の打鍵でかなが確定すれば消えます。
