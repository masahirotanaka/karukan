...
- `core/engine/` — IMEEngine state machine (Empty → Composing → Conversion)
  ...
  - `input_buffer.rs` — Composition record: per-display-char element array (`Romaji`/`Converted`) + caret index; display/reading/pending/raw are derived views
  ...
  - `mode.rs` — Mode switching (katakana, alphabet, live conversion) and Ctrl+L's 英数変換 (`convert_to_alphabet`, `alphabet_forms`)
...
- `input_buf: InputBuffer` in IMEEngine is the source of truth: an element array (one element per display character — `Romaji(char)` unfired keystroke / `Converted { ch, src }` settled character: fired output, passthrough, or direct input) plus a caret index. All views (display, conversion reading, aux romaji tail, raw keystrokes) are derived from it. A settled element keeps `src`, the keystrokes it was typed as, because the kana no longer says how it was spelled (`hello` and `hero` can reach the same element); `attribute_run` cuts a converted run into those sources by replaying it one keystroke at a time, and a rule emitting more than one character at once (`kyo` → きょ) puts the whole run on the first of them. `InputBuffer::raw` concatenates them, which is what Ctrl+L converts back to
...
- The aux line is quiet by default — mode, state, reading, candidate source, page. Both states show the same reading field, the caret's chunk with a `used/max` fill counter (「わせだd 3/30」, `0/30` right after a manual break, making the cut visible): the conversion line calls the composing line's own `aux_reading` (`conversion_reading`), so Space adds the conversion's fields instead of replacing what typing reported, and the chunk is split from the buffer on demand rather than read off `self.chunks` — typing inside a filtered view suppresses the suggestion, which clears the grid. A view that passes its own text (a predictive entry's 「query → reading」) keeps it and only takes the counter. The mode indicator leads the conversion line too (`[A][変換:🤖]`), since a candidate window is open for keystrokes (typing refines, Shift+letter switches to direct input) and nothing else would say which mode is in force. Ctrl+Shift+V (`toggle_verbose`, `[display] verbose` to start that way) adds the debug details, re-rendering the current state's line on the spot — only while something is being typed, since in the Empty state there is no aux line to detail and the chord is the terminal's paste, which has to reach the application: the chunk is swapped for the beam span, labelled 🎯 and counted against `beam_chars` (`🎯 うえ 2/8`), plus inference timing, the model that ran, and the lctx handed to it (`conversion_chunk_reading`; the learning/dictionary views and a selected predictive candidate keep the plain reading, since neither is beamed as a span). `[display] candidate_window = "conversion"` withholds the whole composing window, candidates and aux line both since the aux is drawn in it, so the first window is the one Space opens: applied once at the tail of `process_key` (`hide_candidate_window`), which also empties `shown_suggestions` so Ctrl+digit has nothing to select; the emoji picker is exempt. User-facing doc: `docs/chunking.md`, `docs/configuration.md`
- The exact dictionary match is looked up under the settled reading, and only while no romaji tail is pending (`exact_match_key`): an exact hit on the base would ignore the tail still being typed, offering わせ's entries for 「わせd」. A composition that is *all* unresolved keystrokes (`m`, `gm` — a consonant run no rule reached) has no base to ignore, so the keystrokes themselves are the key, which is what makes a latin shortcut in the user dictionary (read `m` → an address) reachable, the way mozc looks its raw input up alongside the kana. Exact only, never predictive: a one-letter prefix over a latin dictionary would flood the window, and a shortcut is something the user typed in full. A one-character shortcut needs Space, since the first keystroke out of Empty opens the composition without running auto-suggest
- A stranded `n` settles as ん in `flush_pending`, not in the rule table: a bare `n` → ん rule would make `search_longest` match the `n` of `nya` and fire ん before にゃ could form. At the flush nothing can extend it any more, so `nihon` → にほん and `kan` → かん, the way every other Japanese IME ends a word; leaving it literal made one dropped keystroke commit a latin letter into Japanese text. While typing it still waits, so `na`/`nya` are untouched, and `ny` flushed mid-rule is 「んy」
- Word-internal ん cannot be settled that way — かに and かんい are both real — so a dropped `n` becomes a candidate instead (`n_corrected_readings` → the 「もしかして」 annotation). Doubling an `n` in the *keystrokes* is the whole repair, which is what makes it one rule instead of a table of kana pairs: it covers the loss before a vowel (`kanni` → かんい), before な行 (`konnni` → こんに) and before や行 (`shinnya` → しんや) alike, and the buffer already keeps the keystrokes for Ctrl+L. A repair whose reading opens with ん is dropped unweighed: no Japanese word does, and that repair (`nekogasukidesu` → んえこがすきです) is the one the model scored *better* than 猫が好きです. At most `MAX_CORRECTION_VARIANTS` are tried, since each is a model call; a repair converting to the typed reading anyway is dropped, so a clean word costs nothing. The first repair is then *weighed* against the typing (`weigh` → `Verdict`), and only there can it replace what was typed rather than sit under it: both sides are scored as the per-character NLL of the model's own answer to that reading (`KanaKanjiConverter::score`, an `NllScorer` pass), so this compares two conversions the model produced, never a conversion against a guess. The gate is a ratio, not a difference — per-character NLL spans orders of magnitude between a confident reading and one the model was forced through. Over 19 readings on jinen-v2-small, real slips score 3.5–3800× worse than their repair and correct typing stays at or under 1.5×, so `AUTO_CORRECTION_RATIO` = 3 replaces, the mirror `REPAIR_NOISE_RATIO` drops a repair the model disowns (にほんごんおべんきょう at 20× worse), and the band between offers. Erring low silently rewrites what someone typed, so every unknown — no model, a scoring failure, `auto_correct_n` off, a reading under `MIN_AUTO_CORRECTION_CHARS` (かに scores 7.5× worse than かんい only because 蟹 is rarer than 簡易) — resolves to `Offer`, never `Replace`. Only the first repair is weighed, and it reaches the live display too (`live_replacement` → `LiveConversion::correction`, which `live_text` prefers over the chunks' own concatenation): live conversion's job is to show what Enter will commit, so a repair that surfaced only on Space would show the wrong thing for as long as the word was being typed. That costs +11–37ms per keystroke measured, not the ~175ms two scoring passes and a conversion suggest, because the gates come first: a reading under `MIN_AUTO_CORRECTION_CHARS`, and — the big one — any keystroke with a live romaji tail, where `convert_flush(raw) != reading` means the keystrokes say more than the reading does (`sinyabasunin` reads しにゃばすに with an `n` still in hand) and a repair cut from them would be weighed against a conversion of something else. `n_correction_model.rs` holds the `#[ignore]`d model-backed tests that pin all of this. A committed correction learns under the reading that was *typed* (no `with_reading` override) — that is what makes the same slip land right next time, and the repaired reading is one the user already types correctly when they type it at all
...
- A lone Ctrl tap — pressed and released with nothing in between (`track_ctrl_tap`) — comes back to Hiragana, the way the right-hand modifiers and 変換 do; all three share `return_to_hiragana`. Ctrl cannot switch on its *press* the way those keys do, because every Ctrl chord starts with that press and Ctrl+L would flip to kana before its own L arrived; the tap is the gesture that cannot be mistaken for a chord, which is what the macOS frontend already does with right-⌘. Any other key event disarms it, so does a Ctrl pressed under another modifier, and so does `reset` (a Ctrl held across a focus change has no release to wait for). The release is passed through rather than consumed — consuming it would leave the application believing Ctrl is stuck down — which works because the fcitx5 addon applies a result's actions whether or not the key was consumed
- Ctrl+L (`convert_to_alphabet`) hands back the typing as Latin text — mozc's F10/F9, from either Composing or Conversion — and kana input carries straight on, so the Japanese after the Latin word costs no mode key (`hello` + `ka` → 「helloか」): the converted text is settled, and the keystrokes that follow romanize as usual. The switch is one `exit_temporary`, which drops Shift+letter's Alphabet and leaves a mode the user picked outright (Katakana) in place; to keep typing Latin afterwards, Shift+letter opens direct input as it always does. The walk is keyed on the text, not the mode, so it survives that switch. Repeated presses walk `alphabet_forms` (as typed / upper / capitalized, then the full-width pair), always re-cut from the original keystrokes rather than from what is on screen, since upper-casing `HELLO` could never find its way back to `Hello`. Forms that repeat an earlier one drop out, and a press that would leave the text unchanged takes the next form instead, so no press ever looks dead. The walk lives in `alphabet_cycle` and validates itself: a press whose buffer no longer matches what the last one produced starts over, which is how an edit in between ends it without an invalidation hook. Ctrl+Shift+L (live conversion) is taken before dispatch, so only the bare chord reaches the states
