...
//! modifiers do — the way out of the modes Shift+letter and Ctrl+K open.
...
#[test]
fn ctrl_l_needs_no_tap_of_its_own() {
    // Ctrl+L comes back to kana by itself, so the tap has nothing left
    // to do there — and doing it anyway must not disturb the text.
    let mut engine = InputMethodEngine::new();
